...
def get_lat_lng(addr):
    try:
        loc = Nominatim(user_agent="v201_opt").geocode(addr, timeout=10)
...
def sync_save():
    try:
        ...
        for p_n, p_d in st.session_state['projects'].items():
            ...
            # Odchudzamy cache przed zapisem (usuwamy ciężką geometrię)
            ...
            if len(compressed) > 49000:
                st.error(f"⚠️ Projekt '{p_n}' jest za duży dla Google Sheets! Skróć nazwy lub usuń część rejonów.")
                continue
...
def modal_open_project():
    ...
    if st.button("Wczytaj"):
        # Przy wczytywaniu czyścimy cache, żeby geometria przeliczyła się na nowo (bo nie zapisujemy jej)
...
if not st.session_state['data'].empty:
    ...
    with col_main:
        ...
        # Jeśli wczytano projekt, cache może nie mieć 'geom' - trzeba przeliczyć
        active_routes = {}
        for r_n in v_f:
            if r_n in st.session_state['optimized_cache']:
                cache_data = st.session_state['optimized_cache'][r_n]
                # Jeśli brakuje geometrii (bo nie była zapisana), dociągamy ją
    ...
    st.info("Wgraj KML.")import streamlit as st
import pandas as pd
import folium
from streamlit_folium import st_folium
from geopy.geocoders import Nominatim
