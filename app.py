...
st.markdown("""
    ...
    .status-text { font-size: 11px; font-weight: bold; text-align: center; margin-top: 5px; }
...
def parse_kml_robust(file_content):
    ...
    for pm in placemarks:
        name = (re.search(r'<name>(.*?)</name>', pm) or re.search(r'Punkt', 'Punkt')).group(1)
...
with st.sidebar:
    ...
    with st.expander("📁 Projekty i Backup"):
        p_name = st.text_input("Nazwa projektu:")
        if st.button("Zapisz"):
            st.session_state['projects'][p_name] = {'data': st.session_state['data'].copy(), 'start_name': st.session_state['start_name'], 'meta_name': st.session_state['meta_name'], 'start_addr': st.session_state['start_addr'], 'meta_addr': st.session_state['meta_addr']}
        export = {"locs": st.session_state['saved_locations'], "projects": {k: {**v, "data": v["data"].to_dict()} for k, v in st.session_state['projects'].items()}}
        st.download_button("📥 Pobierz Backup", data=json.dumps(export), file_name="backup.json")
...
if not st.session_state['data'].empty:
        ...
        if not st.session_state['start_addr'] or not st.session_state['meta_addr']:
            ...
            with st.spinner("Przetwarzanie danych..."):
                ...
                if ls and lm:
                    ...
                    route = [curr]
                    unvisited = st.session_state['data'].to_dict('records')
                    while unvisited:
    ...
    res_df = st.session_state.get('optimized', st.session_state['data'])

    # 1. MAPA (NA CAŁĄ SZEROKOŚĆ)
    ...
    for i, r in res_df.iterrows():
        ...
        folium.Marker([r['lat'], r['lng']], tooltip=r['Etykieta'], icon=folium.Icon(color=col)).add_to(m)
    ...
    # 2. TABELA (POD MAPĄ - SZEROKA)
    st.markdown("### 📋 Kolejność przystanków")
    # Wyświetlamy tylko kolumnę Etykieta, ale na pełną szerokość
    st.dataframe(res_df[['Etykieta']], use_container_width=True)

else:
    st.info("👈 Wgraj plik KML w panelu bocznym i wybierz bazy, aby rozpocząć.")
