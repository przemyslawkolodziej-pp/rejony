...
<style>
    ...
    button[kind="primary"] { background-color: #28a745 !important; color: white !important; border: 1px solid #ffffff44 !important; }
</style>
    ...
    with st.expander("📍 Bazy", expanded=True):
        if st.session_state['saved_locations']:
            ...
            if sel_b != "---":
                ...
                with c3:
                    if st.button("🗑️", key="d_btn"): del st.session_state['saved_locations'][sel_b]; save_to_disk(); st.rerun()
        st.divider()
    ...
    with st.expander("📁 Projekty"):
        ...
        if st.session_state['projects']:
            ...
            if sel_p != "---":
                ...
                if cd.button("🗑️", key="del_proj"): del st.session_state['projects'][sel_p]; save_to_disk(); st.rerun()
            ...
            if not (sc and mc): st.error("Wybierz Start i Metę!")
                ...
                with st.spinner("Przeliczanie..."):
                    ...
                    for i, group in enumerate(groups):
                        ...
                        st.session_state['geometries'].append({"geom": geom, "color": get_color_key(i), "name": group['source_file'].iloc[0] if mode != "Jedna trasa zbiorcza" else "Całość"})
