...
def check_password():
    if "password" in st.secrets:
        ...
        st.error("Błąd konfiguracji serwera.")
...
if st.sidebar.button("Zapisz projekt"):
    if p_name:
...
with st.sidebar.expander("Dodaj bazę"):
...
up_kml = st.sidebar.file_uploader("KML", type=['kml'])
if up_kml and st.sidebar.button("Wczytaj KML"):
...
if not df.empty:
    if st.button("🚀 OPTYMALIZUJ", type="primary", use_container_width=True):
        geolocator = Nominatim(user_agent="opt_v23")
    ...
    with cr:
        ...
        if not m_df.empty:
            ...
            st_folium(m, width="100%", height=600, key="map_v23")
