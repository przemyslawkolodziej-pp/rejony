...
def get_lat_lng(address):
    try:
        gl = Nominatim(user_agent="v130_geo")
...
def parse_kml(content, name):
    ...
    for pm in placemarks:
        ...
        if c:
            pts.append({"display_name": n.group(1) if n else "Punkt", "lat": float(c.group(2)), "lng": float(c.group(1)), "source_file": name})
...
with st.sidebar:
    ...
    with st.expander("☁️ Wgrywanie KML", expanded=False):
        ...
        if up and st.button("Wczytaj dane"):
            all_pts = []
            for f in up:
                df = parse_kml(f.read().decode('utf-8'), f.name)
                all_pts.append(df)
            if all_pts:
    ...
    with st.expander("🏠 Zarządzanie bazami", expanded=True):
        ...
        with st.form("new_b", clear_on_submit=True):
            n, a = st.text_input("Nazwa bazy:"), st.text_input("Adres:")
...
if not st.session_state['data'].empty:
    ...
    with col_filters:
        u_files = sorted(st.session_state['data']['source_file'].unique().tolist())
        v_files = st.multiselect("Widoczne rejony:", u_files, default=u_files, key=f"ms_{st.session_state['reset_counter']}")
    ...
    with col_view:
        ...
        mode = st.radio("Tryb obliczeń:", ["Jedna trasa (wszystkie punkty)", "Oddzielne trasy dla rejonów"], horizontal=True)

    col_calc, col_reset = st.columns([4, 1])
    with col_calc:
        if st.button("🚀 OBLICZ OPTYMALNE TRASY", type="primary", use_container_width=True):
            if not (sc and mc):
                st.error("Ustaw punkt START i META w panelu bocznym!")
            else:
                with st.spinner("Trwa obliczanie tras..."):
                    st.session_state['optimized_list'] = []
                    st.session_state['geometries'] = []
                    
                    if mode == "Jedna trasa (wszystkie punkty)":
                        groups = [filtered_df]
                    else:
                        groups = [filtered_df[filtered_df['source_file'] == f] for f in filtered_df['source_file'].unique()]

                    for group in groups:
                        if group.empty: continue
                        source_name = group['source_file'].iloc[0] if mode != "Jedna trasa (wszystkie punkty)" else "Wszystkie rejony"
                        route_color = file_color_map.get(group['source_file'].iloc[0], 'green') if mode != "Jedna trasa (wszystkie punkty)" else 'green'
                        ...
                        while unvisited:
                            next_pt = min(unvisited, key=lambda x: get_math_dist(curr['lat'], curr['lng'], x['lat'], x['lng']))
                        ...
                        geom, dist, time_sec = get_route_chunked([[r['lat'], r['lng']] for r in route])
                        st.session_state['optimized_list'].append(pd.DataFrame(route))
                        st.session_state['geometries'].append({
                            "geom": geom, "color": route_color, "dist": dist, "time": time_sec,
                            "pts_count": len(group), "name": source_name, "source_file": group['source_file'].iloc[0] if mode != "Jedna trasa (wszystkie punkty)" else "ALL"
                        })
                    st.rerun()
    ...
    with col_reset:
        if st.button("✖ WYCZYŚĆ WSZYSTKO", use_container_width=True):
            st.session_state.update({
                'optimized_list': [], 'geometries': [], 'data': pd.DataFrame(),
                'start_name': "Nie wybrano", 'meta_name': "Nie wybrano",
                'start_coords': None, 'meta_coords': None, 'reset_counter': st.session_state['reset_counter'] + 1
            })
            st.rerun()
    ...
    m = folium.Map()
    all_coords = []
    if sc: all_coords.append([sc['lat'], sc['lng']])
    ...
    if mc: folium.Marker([mc['lat'], mc['lng']], icon=folium.Icon(color='red', icon='flag', prefix='fa'), tooltip="META").add_to(m)
    
    visible_geoms = [g for g in st.session_state['geometries'] if mode == "Jedna trasa (wszystkie punkty)" or g.get('source_file') in v_files]
    ...
    if show_pins:
        for _, r in filtered_df.iterrows():
            folium.Marker(
                [r['lat'], r['lng']], 
                icon=folium.Icon(color=file_color_map.get(r['source_file'], 'gray'), icon='circle', prefix='fa'),
                tooltip=f"{r['display_name']} ({r['source_file']})"
            ).add_to(m)
    
    st_folium(m, width="100%", height=550, key=f"map_{st.session_state['reset_counter']}")
    
    if st.session_state['geometries']:
        total_dist = sum(g['dist'] for g in visible_geoms)
        total_time = sum(g['time'] for g in visible_geoms)
        st.success(f"📊 **PODSUMOWANIE:** Dystans: {total_dist/1000:.2f} km | Szacowany czas: {int(total_time//3600)}h {int((total_time%3600)//60)}min")
else:
    if sc or mc:
        st.info("📍 Wybrano bazy. Wgraj pliki KML, aby zobaczyć punkty na mapie.")
        m = folium.Map()
        pts = []
        if sc: 
        ...
        st.info("👈 Zacznij od wgrania plików KML i wybrania bazy w panelu bocznym.")
