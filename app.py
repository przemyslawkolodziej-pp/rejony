...
# --- 2. FUNKCJE BEZPIECZEŃSTWA (TOKEN URL) ---
def generate_session_token(password):
    """Tworzy bezpieczny hash z hasła, aby nie przekazywać go jawnym tekstem."""
    return hashlib.sha256(password.encode()).hexdigest()

def check_auth():
    """Sprawdza czy użytkownik jest zalogowany (w sesji lub przez URL)."""
    ...
    with st.form("login"):
        ...
        if st.form_submit_button("Zaloguj"):
            if pwd == st.secrets.get("password"):
                token = generate_session_token(pwd)
                st.query_params["token"] = token # Zapisz token w URL
                st.session_state['authenticated'] = True
                sync_load()
                st.rerun()
...
# Jeśli jesteśmy tu, znaczy że użytkownik jest zalogowany
if not st.session_state['projects'] and st.session_state['authenticated']:
    sync_load() # Doczytaj dane po autologowaniu z URL

# --- 5. STYLE I POMOCNICZE ---
st.markdown("<style>div.stButton > button { border-radius: 8px; } button[kind='primary'] { background-color: #28a745 !important; }</style>", unsafe_allow_html=True)

def get_lat_lng(addr):
    try:
        loc = Nominatim(user_agent="v150").geocode(addr, timeout=10)
...
with st.sidebar:
    ...
    with st.expander("📁 Projekty", expanded=True):
        p_in = st.text_input("Nazwa projektu:")
        if st.button("💾 Zapisz Projekt") and p_in:
    ...
    with st.expander("🏠 Bazy", expanded=True):
        if st.session_state['saved_locations']:
            sel_b = st.selectbox("Bazy:", ["---"] + sorted(st.session_state['saved_locations'].keys()))
            if sel_b != "---":
    ...
    if st.button("🔓 WYLOGUJ"):
        st.query_params.clear() # Czyści token z URL
        st.session_state.update({'authenticated': False, 'projects': {}, 'data': pd.DataFrame()})
        st.rerun()
...
if not st.session_state['data'].empty:
        ...
        if not (st.session_state['start_coords'] and st.session_state['meta_coords']): st.error("Ustaw Start i Metę!")
            ...
            with st.spinner("Liczenie..."):
                ...
                for idx, g in enumerate(grps):
                    ...
                    st.session_state['geometries'].append({"geom": geom, "color": ['blue','red','green','orange','purple'][idx%5], "dist": d, "time": t, "name": g['source_file'].iloc[0] if mode != "Jedna trasa" else "Wszystkie", "pts": len(g)})
                st.rerun()
    ...
    if st.session_state['geometries']:
        ...
        td, tt = sum(g['dist'] for g in st.session_state['geometries']), sum(g['time'] for g in st.session_state['geometries'])
